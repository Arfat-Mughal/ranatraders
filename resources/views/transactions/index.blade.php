...
@section('content')
    ...
    <div class="row mb-3">
        ...
        <div class="col-lg-2">
            ...
            <select id="from_customer" name="from_customer" class="form-select">
                <option value="" disabled selected>Select a Customer</option>
                @foreach ($customers as $customer)
                    <option value="{{ $customer->id }}" {{ request('from_customer') == $customer->id ? 'selected' : '' }}>
        ...
        <div class="col-lg-2">
            ...
            <select id="to_customer" name="to_customer" class="form-select">
                <option value="" disabled selected>Select a Customer</option>
                @foreach ($customers as $customer)
                    <option value="{{ $customer->id }}" {{ request('to_customer') == $customer->id ? 'selected' : '' }}>
                        {{ $customer->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-lg-2">
            <label class="form-label">&nbsp;</label>
            <a href="{{ route('transactions.index') }}" class="btn btn-secondary w-100">Clear Filters</a>
        </div>
    ...
    <script>
        ...
        function confirmEdit() {
            return confirm('Be careful, Activities are recorded. Still need to update this transaction?');
        }
    </script>
